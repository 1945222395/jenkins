...
init_war_backup () {
[ -d ${war_bak_path} ] || mkdir -p ${war_bak_path}
judge=`ls ${init_app_path}/${war_name}*.war | wc -l`
if [ ${judge} -gt 0 ];then
	old_war_name=`cd ${init_app_path}; ls ${war_name}*.war | awk -F '.war' '{print $1}'`
...
init_war_deploy () {
	cd ${war_path} ; cp ${war_all_name} ${init_app_path};
...
update)
	Clean_backups && init_war_backup && init_war_deploy && health_check && Post_steps
;;
rollback)
